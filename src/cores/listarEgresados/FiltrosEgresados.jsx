...
function FiltrosEgresados() {
  ...
  const isDisabled = !valueName && list.length === 0 && listPos.length === 0 && Object.keys(selectedTags).every(tag => !selectedTags[tag]);
  ...
  const handleSubmit = () => {
    ...
    const data = {
      ...
      selectedTags: Object.keys(selectedTags).filter(
      ...
      <Drawer placement={placement} onClose={onClose} isOpen={isOpen} size="md">
        ...
        <DrawerContent paddingLeft="5px" paddingRight="5px">
          ...
          >
            ...
            {isLargerThan435 ? (<Stack p={{ base: 4, md: "20 20 10 20" }}>
            ...
            </Stack>) : (<Stack p={{ base: 4, md: "20 20 10 20" }}>
              <Flex direction="row" justifyContent="center" wrap="wrap">
                {labels.map((label) => (
                  <Tag
                    key={label}
            ...
            {isLargerThan435 ? (<><Button
              ...
              <Tooltip label="Para buscar, complete al menos un filtro" isOpen={isDisabled && isHovering}>
              <Button
