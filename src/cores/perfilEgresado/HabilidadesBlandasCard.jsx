...
import { AddSoftSkills, DeleteSoftSkills, editSoftSkill, getSoftSkills} from "../../services/auth/MeProfile.services";
...
const [newCardData, setNewCardData] = useState(cardData);
  ...
  const [cardIdToEditSoftSkill, setcardIdToEditSoftSkill] = useState(null);
  ...
  if (cardExists) {
    ...
    const newCard = await AddSoftSkills(newData);
...
const handleEditCard = async (cardTitle) => {
  const cardToEdit = await getSoftSkills(cardTitle);
...
const handleEditInputChange = (field, value, setState) => {
  ...
  const updatedCard = await editSoftSkill(originalSkillName, newData);
  ...
  const handleConfirmDelete = async (cardToDelete, cardTypeToDelete) => {
    if (cardToDelete !== null && cardTypeToDelete !== null) {
      if (cardTypeToDelete === "cardContent") {
        await DeleteSoftSkills(cardToDelete);
  ...
  {Array.isArray(newCardData) && newCardData.length > 0
    ...
    <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)}>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button colorScheme="blue" mr={3} onClick={handleAddSoftSkill}>
              Guardar
            </Button>
